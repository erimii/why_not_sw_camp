...
os.environ["PATH"] += os.pathsep + r"C:\Users\Admin\PycharmProjects\20250520\ffmpeg-2025-05-15-git-12b853530a-full_build\bin"

import torch
from transformers import AutoModelForSpeechSeq2Seq, AutoProcessor, pipeline

### 받아쓰기하는 함수 whisper_stt 만들기
...
def stt_to_rttm(
    ...
    """
    그 후 가져온 시간대별로 어느 화자가 발언했는지 정리된 df_rttm에
    "text"라는 빈 열을 추가하고 Whisper 모델로 받아온 텍스트를 채운다.
    """
    df_rttm["text"] = ""

    """
    중요한 부분은 for 문을 사용해 위스퍼 모델로 받아쓴 결과인 df_stt의
    'start', 'end', 'text' 정보를 이용해 df_rttm의 적절한 행을 찾는다
    """
    for i_stt, row_stt in df_stt.iterrows():
        overlap_dict = {}

        '''
        df_stt 행과 df_rttm 행이 겹치는 시간을 각각 계산해서 가장 많이 겹치는 행에 텍스트를 추가.
        '''
    ...
    df_rttm.to_csv(
        ...
        index=False,  # 인덱스는 저장하지 않음
        sep='|',
        encoding='utf-8'  # 결과는 지정된 경로에 저장됨
    )
